...
# Function to get sentiment features using VADER
def get_sentiment_features(texts, sia):
    features = []
    for text in texts:
...
class SVM:
    ...
    # Compute the decision function using einsum
        decision_function = np.dot(X, support_vectors_transposed)  # This line is changed
...
# Load the dataset
dataset = pd.read_csv('waisnawa.csv')  # Replace with your dataset file
...
languages = [detect_language(texts) for texts in texts]
...
#--------------------------------------------------------------------------------------------------------------------------------#re
#------------------------------------------------HASHTAG ANALYSIS, EXTRACT MOST COMMON HASHTAG USED------------------------------#
#--------------------------------------------------------------------------------------------------------------------------------#

def extract_hashtags(text):
    return re.findall(r'#\w+', text)

# Apply the function to each post and create a list of all hashtags
all_hashtags = [hashtag for post in texts for hashtag in extract_hashtags(post)]


hashtag_counter = Counter(all_hashtags)
most_common_hashtags = hashtag_counter.most_common(10)  # Adjust number as needed
...
"""
lexicon_dict = sia.lexicon

for word, score in lexicon_dict.items():
    print(word, score)
    
# To print first 10 items in the dictionary
for word in list(lexicon_dict)[:10]:
    print(word, lexicon_dict[word])
"""
...
svm_classifier = SVM()
svm_classifier.fit(X_train_combined, labels_train_svm)

#dump(svm_classifier, 'svm_classifier4444.joblib') 

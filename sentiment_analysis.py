...
class SentiText(object):
    ...
    def __init__(self, text):
        if not isinstance(text, str):
            text = str(text).encode('utf-8') #convert the input if text is not a string
        self.text = text
    ...
    def _words_and_emoticons(self):
        ...
        """
        removed_emoticons = self.text.split()
        stripped = list(map(self._strip_punc_if_word, removed_emoticons))
        return stripped
...
# Function to get sentiment feature
